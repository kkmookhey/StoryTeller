...
4. **Score** in ONE batched call via `references/scoring-rubric.md` with `kk-voice` loaded. Drop `score < 4`. Sort desc. Keep top-N from `config.scoring.top_n`. Note: the kept set IS what gets drafted — top_n controls volume, not quality threshold. A thin week may yield top drafts at score 4-6; that's expected and KK reviews in Postiz.
5. **Draft** each top signal in every `enabled: true` format per matching `references/drafting-*.md` (drafters accept any score >=4). On `status: "error"` with routing hint, log and continue; without one, save input to `~/.storyteller/failed-pushes/<signal_id>-<format>.json`.
...
7. **Publish:** for each NOT `hold: true`, invoke Postiz CLI to create a **draft** (never published) per `references/publish-postiz.md`. For `hold: true`, save to `~/.storyteller/pending-video/<safe_signal_id>-<platform>.json` where `safe_signal_id` replaces filesystem-unfriendly chars (`:` `/` `#`) with `_` — e.g., `github:kkmookhey/ciso-copilot:pr#18` → `github_kkmookhey_ciso-copilot_pr_18`. Capture draft IDs. Retry once on failure; second failure moves to `~/.storyteller/failed-pushes/` (same encoding).
